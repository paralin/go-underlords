...
func GenerateAPI(ctx context.Context, clientOutput, eventsOutput io.Writer) error {
	senderMap := make(map[gcm.EGCDACClientMessages]MsgSender)
	...
	fmt.Fprintf(clientOutput, "package dac\n\nimport (\n")

	msgIds := getSortedMsgIDs()
	var requestFuncs []*generatedRequestFunc

	eventsImports := make(map[string]struct{})
	eventsImports["github.com/paralin/go-underlords/protocol"] = struct{}{}
	eventsImports["github.com/golang/protobuf/proto"] = struct{}{}

	clientImports := make(map[string]struct{})
	clientImports["context"] = struct{}{}
	clientImports["github.com/paralin/go-underlords/protocol"] = struct{}{}
	// clientImports["github.com/faceit/go-steam/steamid"] = struct{}{}
	clientImports["github.com/paralin/go-underlords/events"] = struct{}{}

	// responseMsgs are messages that are known to be responses.
	responseMsgs := make(map[gcm.EGCDACClientMessages]struct{})
	eventHandlers := make(map[gcm.EGCDACClientMessages]*generatedEventHandler)
	...
	for _, f := range requestFuncs {
		fmt.Fprintf(clientOutput, "\n// %s\n", f.generateComment())
		fmt.Fprintf(clientOutput, "func (d *DAC) %s(\n", f.methodName)
		...
		if respTyp != nil {
			...
			fmt.Fprintf(clientOutput, "\t\tctx,\n\t\tuint32(protocol.EGCDACClientMessages_%s),\n", f.reqMsgID.String())
			fmt.Fprintf(clientOutput, "\t\treq,\n\t\tuint32(protocol.EGCDACClientMessages_%s),\n", f.respMsgID.String())
			fmt.Fprintf(clientOutput, "\t\tresp,\n\t)\n")
		} else {
			fmt.Fprintf(clientOutput, "\td.write(uint32(protocol.EGCDACClientMessages_%s), req)\n", f.reqMsgID.String())
		}
	...
	fmt.Fprintf(clientOutput, "func (d *DAC) registerGeneratedHandlers() {\n")
	...
	for _, eventHandler := range eventHandlersOrdered {
		...
		fmt.Fprintf(eventsOutput, "func (e *%s) GetDotaEventMsgID() protocol.EGCDACClientMessages {\n", eventHandler.eventName)
		fmt.Fprintf(eventsOutput, "\treturn protocol.EGCDACClientMessages_%s\n", eventHandler.msgID.String())
		fmt.Fprintf(eventsOutput, "}\n")
		...
		fmt.Fprintf(
			clientOutput,
			"\td.handlers[uint32(protocol.EGCDACClientMessages_%s)] = d.getEventEmitter(func() events.Event {\n",
			eventHandler.msgID.String(),
		)
