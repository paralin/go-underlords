...
func GetResponseMessageID(reqID gcm.EGCDACClientMessages) (gcm.EGCDACClientMessages, error) {
	...
	queryRespStr := func(respStr string) (gcm.EGCDACClientMessages, bool) {
		val, ok := gcm.EGCDACClientMessages_value["k_EMsg"+respStr]
		if ok {
			return gcm.EGCDACClientMessages(val), true
		}

		if clientToGC {
			val, ok = gcm.EGCDACClientMessages_value["k_EMsgGCToClient"+respStr]
			if ok {
				return gcm.EGCDACClientMessages(val), true
			}

			val, ok = gcm.EGCDACClientMessages_value["k_EMsgClientToGC"+respStr]
			if ok {
				return gcm.EGCDACClientMessages(val), true
			}
		}

		return gcm.EGCDACClientMessages(0), false
	}
	...
	if responseImplied {
		...
		return gcm.EGCDACClientMessages(0), errors.Errorf("response was implied by request %v but no response type found", msgID.String())
	}

	return gcm.EGCDACClientMessages(0), nil
}

// LookupMessageProtoType lookup proto from message ID.
func LookupMessageProtoType(protoMap map[string]*ProtoType, msgID gcm.EGCDACClientMessages) (*ProtoType, error) {
